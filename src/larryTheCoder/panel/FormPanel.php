...
class FormPanel implements Listener {
    ...
    private function setupSpawn(Player $player, SkyWarsData $arena = null) {
        ...
        $this->setters[strtolower($player->getName())]['arena'] = $arena->arenaName;
        ...
        $this->cleanupArray($player);
    }

    private function cleanupArray(Player $player) {
        unset($this->data[$player->getName()]);
        if (isset($this->arenas[$player->getName()])) {
            $this->arenas[$player->getName()]->setup = false;
            unset($this->arenas[$player->getName()]);
        }
    ...
    private function joinSignSetup(Player $player, SkyWarsData $data) {
        ...
        $this->setters[strtolower($player->getName())]['arena'] = $data->arenaName;
        $player->sendMessage($this->plugin->getMsg($player, 'panel-spawn-wand'));
        $player->getInventory()->setItemInHand(new BlazeRod());
        $this->cleanupArray($player);
    }
    ...
    public function onBlockBreak(BlockBreakEvent $e) {
        ...
        if (isset($this->setters[strtolower($p->getName())]['arena']) && isset($this->setters[strtolower($p->getName())]['type'])) {
            if (!is_null($e->getItem()) && $e->getItem()->getId() === Item::BLAZE_ROD) {
                ...
                $e->setCancelled(true);
                $b = $e->getBlock();
                $arena = new ConfigManager($this->setters[strtolower($p->getName())]['arena'], $this->plugin);
