...
class Utils {
    ...
    public static function unloadGame() {
        foreach (SkyWarsAPI::getInstance()->ins as $arena) {
            if ($arena->game !== 0) {
                $arena->stopGame();
            }
        }
    ...
    public static function checkFile(Config $arena) {
        if (!(is_numeric($arena->getNested("signs.join_sign_x")) 
                ...
                && is_string($arena->getNested("arena.weather"))     
